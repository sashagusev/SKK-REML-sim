truth = read.table("sim_noninf.out",as.is=T)
...
rand = read.table("run_random_noninf.out",as.is=T)
P = 50000
P.samp = 5000

svg("plot_krishna_kumar_pnas.noninf.svg",height=4)
...
plot( dens , lwd=2 , xlab="per-SNP heritability" , ylab="density" , main="500 random h2 estimates from 5k SNPs [10% causal]" , xlim=xlim , bty="n"  )
# shade the middle 95%
